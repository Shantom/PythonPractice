...
//#define 0 0
//
////#define "\033[0m" "\033[0m"
////#define "\033[1;4;22;34m"  "\033[1;4;22;34m"
...
int main (int argc, const char *argv[]) {
	...
	if (args == 1) {
		...
		if (current) {                               // Add comment to the current directory
			// char *file;
			// file = "CURRENT";
			addComment(argv[1], cwd, argv[2], 0);
		...
		if (current && push) { // Push comment to the current directory
			...
			addComment(argv[1], cwd, argv[3], 1);
...
void checkComment(const char *file, const char *path) {
	...
	if (dirOrFileExists(file)) {
		...
		if (dirOrFileExists(dir)) {
			...
			printf("\033[1;4;22;34m" "%s" "\033[0m" "\n", file);
		}
...
void printCurrentComment(const char *path) {
	...
	char *fin = NULL;
	asprintf(&fin, "%s/" ".comment" "/" "." ".comment", path);
	...
	if (dirOrFileExists(fin)) {
		...
		printf("\033[1;4;22;34m" "Current Directory:" "\033[0m" "\t");
		put_multiline(newch, 50);
		printf("\n");

		fclose(fp);
	} else {
		printf("\033[1;4;22;34m" "Current directory" "\033[0m" "\thas no comment\n");
	}
...
void printComment(const char *filename, const char *path) {
	...
	char newch[1000];
	fp = fopen(path, "r");
	printf("\033[1;4;22;34m" "%s", filename);
	printf("\033[0m" "");
	int i = 0;
	printf("\t");
	while (1) {
...
void printUsage() {
	fputs("\033[1;4;22;34m" "c\n" "\033[0m""  by @Jonovono - Add comments to files or directories.\n\n", stderr);
	fputs("Usage:\n", stderr);
